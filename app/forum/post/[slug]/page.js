...
export default async function ForumPostDetailPage({ params }) {
  ...
  const safeHtml = sanitizeForumHtml(post.body_html || "");
            ...
            {user.id === post.author_id ? (
              <div className="mt-8">
                <Link
                  ...
                  className="inline-flex rounded-2xl border border-[#D7E1E8] bg-[#F8FBFC] px-5 py-3 text-sm font-semibold text-[#12212B]"
                >
                  Edit post
                </Link>
              </div>
        ...
        <section className="rounded-[28px] border border-[#D7E1E7] bg-white p-6 shadow-[0_12px_35px_rgba(23,43,58,0.06)] sm:p-8">
          ...
          {reviews.length ? (
            ...
            <div className="mt-6 rounded-[20px] border border-dashed border-[#D7E1E8] bg-[#FAFCFD] p-5 text-sm text-[#61727E]">
              No completed-exchange reviews yet for this thread.
            </div>
          )}
        </section>
      </div>
